const conferenceApiGetResponse = {
  name:
    'Ratione sapiente ut omnis deleniti veritatis aut occaecati et nihil. Delectus non aut rerum voluptas eligendi sint distinctio. Et sed fuga nesciunt nobis non et enim quas provident. Quia voluptatum est ad recusandae. Quis est excepturi quam consequatur fugit rerum accusamus.',
  location:
    'Et sed officiis voluptas libero. Molestiae quia voluptatem delectus nesciunt. Quibusdam quisquam velit qui aut. Voluptatem rerum rem velit et pariatur laudantium blanditiis soluta nesciunt. Explicabo adipisci rerum doloribus quo.',
  date: '2005-11-21',
};

export default conferenceApiGetResponse;
